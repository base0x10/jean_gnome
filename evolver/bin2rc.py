...
def convert_instr(data):
    ...
    # 5 bits
    opcodes = [ "DAT", "MOV", "ADD", "SUB", "MUL", "DIV",
                "MOD", "JMP", "JMZ", "JMN", "DJN", "SPL",
                "SLT", "CMP", "SEQ", "SNE", "NOP"]
    # 3 bits
    ...
    instr_number >>= 3
    return "%s.%s %s%d, %s%d\n" % (op_str, modifier_str, modes_str_a, a_val, modes_str_b, b_val)

def convert_file(mm):
    org = mm[0] % 100
    lines = [
            convert_instr(mm[1 + 6*i : 1 + i+6*(i+1)]) 
